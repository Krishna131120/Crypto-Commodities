"""
Fetch ALL available symbols from Binance (mainnet or testnet) and add them to symbol_universe.py.

This script:
1. Fetches ALL active spot trading pairs from Binance API (mainnet or testnet)
...
5. Supports both mainnet and testnet (paper trading)

Usage:
    python binance/fetch_all_binance_symbols.py              # Use mainnet
    python binance/fetch_all_binance_symbols.py --testnet     # Use testnet (paper trading)
"""
...
def fetch_binance_trading_pairs(use_testnet: bool = False) -> List[Dict[str, Any]]:
    """
    Fetch ALL active spot trading pairs from Binance API (mainnet or testnet).
    
    Args:
        use_testnet: If True, use Binance testnet API (paper trading)
    ...
    if use_testnet:
        ...
        print("Fetching trading pairs from Binance TESTNET (paper trading) API...")
    else:
        url = "https://api.binance.com/api/v3/exchangeInfo"
        print("Fetching trading pairs from Binance MAINNET API...")
...
def add_symbols_to_universe(new_symbols: List[str]) -> int:
    ...
    new_content += "\n"
    new_content += "    # Binance trading pairs (auto-generated from paper trading)\n"
    new_content += "\n".join(new_symbols)
    new_content += "\n"
    new_content += "\n".join(lines[insert_idx:])
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch ALL Binance trading pairs and add them to symbol_universe.py"
    )
    parser.add_argument(
        "--testnet",
        action="store_true",
        help="Use Binance testnet (paper trading) API instead of mainnet",
    )
    args = parser.parse_args()
    
    print("=" * 80)
    print("FETCHING ALL BINANCE TRADING PAIRS")
    print("=" * 80)
    print()
    ...
    if not new_symbols:
        ...
        print(f"Total symbols ready for trading: {len(pairs)}")
        return
    ...
    print(f"[INFO] Total symbols ready for Binance paper trading: {len(pairs)}")
    print("\nNext steps:")
    ...
    print("  4. Start trading: python binance/end_to_end_binance.py --profit-target 1.5")
    print("=" * 80)
